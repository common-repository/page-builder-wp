 ...
 * This class optimises CSS data generated by pbwpcss.
 *
...
class pbwpcss_optimise
{
    ...
    /**
     * Constructor
     * @param array $css contains the class pbwpcss
     * @access private
     * @version 1.0
     */
    ...
    {
        $shorthands = &$this->parser->data[ 'pbwpcss' ][ 'shorthands' ];
    ...
    {
        $replace_colors = &$this->parser->data[ 'pbwpcss' ][ 'replace_colors' ];
    ...
    {
        $unit_values  = &$this->parser->data[ 'pbwpcss' ][ 'unit_values' ];
        $color_values = &$this->parser->data[ 'pbwpcss' ][ 'color_values' ];
    ...
    public function AnalyseCssNumber( $string )
    {
        ...
        $units  = &$this->parser->data[ 'pbwpcss' ][ 'units' ];
        $return = [ 0, '' ];
    ...
    {
        $shorthands = &$this->parser->data[ 'pbwpcss' ][ 'radius_shorthands' ];
        if ( ! is_array( $shorthands[ $property ] ) ) {
            $return[ $property ] = $value;
    ...
    public function dissolve_short_bg( $str_value )
    {
        ...
        $background_prop_default = &$this->parser->data[ 'pbwpcss' ][ 'background_prop_default' ];
    ...
    public function merge_bg( $input_css )
    {
        ...
        $backgroundImageCount = max(
            count( $this->explode_ws( ',', $input_css[ 'background-image' ] ) ),
            count( $this->explode_ws( ',', $input_css[ 'background-color' ] ) ),
            1
        );
        ...
        // if background properties are here and not empty, don't try anything
        if ( isset( $input_css[ 'background' ] ) && $input_css[ 'background' ] ) {
            return $input_css;
        }

        for ( $i = 0; $i < $backgroundImageCount; $i++ ) {
            foreach ( $background_prop_default as $bg_property => $default_value ) {
                ...
                // skip all optimization if gradient() somewhere
                ...
                if (
                    ( ! isset( $bg_img_array[ $i ] ) || $bg_img_array[ $i ] === 'none' )
                    && (
                        $bg_property === 'background-size'
                        || $bg_property === 'background-position'
                        || $bg_property === 'background-attachment'
                        || $bg_property === 'background-repeat'
                    )
                ) {
                    continue;
                }
    ...
    {
        $font_prop_default = &$this->parser->data[ 'pbwpcss' ][ 'font_prop_default' ];
    ...
    public function reverse_left_and_right_background_position( $value )
    {
        ...
        if ( stripos( $value, 'left' ) === false and stripos( $value, 'right' ) === false ) {
            ...
            if ( count( $values ) == 1 ) {
                if ( in_array( $value, [ 'center', 'top', 'bottom', 'inherit', 'initial', 'unset' ] ) ) {
                    return $value;
                }

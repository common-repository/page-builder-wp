...
/**
 * PBWPCss - CSS Parser and Optimiser
 *
 * CSS Parser class
 *
...
/**
 * Defines constants
 * @todo //TODO: make them class constants of pbwpcss
 */
define('AT_START',         1);
define('AT_END',           2);
define('SEL_START',        3);
 ...
 * @version 1.1.0
 */
require(__DIR__ . DIRECTORY_SEPARATOR . 'class.pbwpcss_print.php');
 ...
 * @version 1.0
 */
require(__DIR__ . DIRECTORY_SEPARATOR . 'class.pbwpcss_optimise.php');
...
class pbwpcss {
	...
	public $namespace = '';
	/**
	 * Contains the version of pbwpcss
	 * @var string
	 * @access private
	 */
	...
	public function __construct() {
		...
		$this->optimise = new pbwpcss_optimise($this);

		$this->tokens_list = & $this->data['pbwpcss']['tokens'];
	}
	...
	public function write($filename, $formatted=false, $doctype='xhtml1.1', $externalcss=true, $title='', $lang='en', $pre_code=true) {
		...
		if (!is_dir('temp')) {
			$madedir = mkdir('temp');
			if (!$madedir) {
				print esc_html('Could not make directory "temp" in ' . dirname(__FILE__) );
				exit;
			}
		}
	 ...
	 * @access public
	 * @version 1.1
	 * @see http://pbwpcss.sourceforge.net/templates.php
	 */
	public function load_template($content, $from_file=true) {
		$predefined_templates = & $this->data['pbwpcss']['predefined_templates'];
		...
		if ($from_file) {
			$content = wp_strip_all_tags(file_get_contents($content), '<span>');
		}
	...
	public function parse($string) {
		...
		$this->print = new pbwpcss_print($this);
		$this->optimise = new pbwpcss_optimise($this);

		$all_properties = & $this->data['pbwpcss']['all_properties'];
		$at_rules = & $this->data['pbwpcss']['at_rules'];
		$quoted_string_properties = & $this->data['pbwpcss']['quoted_string_properties'];
			...
			switch ($this->status) {
				...
				case 'instr':
					...
					if ($string[$i] === $_str_char && !$this->escaped($string, $i)) {
						...
						if (!preg_match('|[' . implode('', $this->data['pbwpcss']['whitespace']) . ']|uis', $_cur_string) && $this->property !== 'content') {
							if (!$_quoted_string) {
								if ($_str_char !== ')') {
	...
	static function escaped(&$string, $pos) {
		return!(($string[$pos - 1] !== '\\') || pbwpcss::escaped($string, $pos - 1));
	}
	...
	public function is_important(&$value) {
		return (
			...
			AND !strcasecmp(substr(str_replace($this->data['pbwpcss']['whitespace'], '', $value), -10, 10), '!important'));
	}
	...
	public function property_is_valid($property) {
		...
		elseif (in_array(trim($property), $this->data['pbwpcss']['multiple_properties'])) {
